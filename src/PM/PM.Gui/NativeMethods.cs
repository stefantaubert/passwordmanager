...
namespace PM.Gui
{
    internal static class NativeMethods
    {
        ...
        /// <param name="cueBanner">Der CueBanner.</param>
        /// <param name="hide">true, wenn der CueBanner entfernt werden soll, sonst false.</param>
        public static void SetCueBanner(this IntPtr handle, string cueBanner, bool hide)
        {
            if (!string.IsNullOrWhiteSpace(cueBanner) || hide)
            {
                NativeMethods.SendMessageW(handle, NativeMethods.EM_SETCUEBANNER, default(IntPtr), cueBanner);
            }
        }
    }
}